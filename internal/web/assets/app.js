...
function fileBlock(entry, index) {
  ...
  fentry.compareGo.addEventListener("click", () => el.compareDialog.open(entry.infohash, index));
...
function frameFigure(frame, fentry) {
  ...
  const open = () => { if (frame.url) el.framePanel.open(img.src, caption.textContent); };
