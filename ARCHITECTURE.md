...
- **`swarm`** — вся работа с anacrolix: сессия, приоритеты, доступность, счётчик байт и
  **пул** (`swarm.Pool`) — один долгоживущий клиент на все публичные раздачи, из которого
  запуск берёт раздачу (`Attach`) и в который её возвращает (`Detach`). Приватная раздача и
  раздача с ещё неизвестной приватностью получают клиента себе — это и есть граница, за
  которую BEP 27 не пускает. Владеет пулом тот, кто его создал: сервер, а не запуск.
...
3. **Порты BitTorrent — набор, а не порт.** Порт принадлежит клиенту anacrolix (TCP, uTP
   ...
   публичной. Обратная сторона того же факта: все публичные раздачи помещаются в один
   клиент и стоят один порт на всех, сколько бы их ни было (`swarm.Pool`).
   Значит клиентов больше одного, портов тоже, и раздавать их должен кто-то
